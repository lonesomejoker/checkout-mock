              ...
              <section className="flex text-violet-500 bg-neutral-800 px-3 py-2 gap-x-2 items-center justify-center" onClick={() => setIsClicked(item.id)}>
                {isClicked===item.id ? (
                  ...
                  <>
                    ...
                    <PiShoppingCartSimpleBold className="text-sm md:text-2xl" />
                  </>
                )}
              </section>
              {isClicked === item.id && (
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-x-0.5 md:gap-x-2 gap-y-1.5 px-2">
                    {['M', 'L', 'XL', '2XL', '3XL'].map((size) => (
                      <button key={size}
                        className={`py-2.5 duration-300 ${selectedSize === size ? "bg-indigo-500 text-white" : "bg-gray-200 text-black"} backdrop-blur rounded`}
                        onClick={() => setSelectedSize(size)}>
                        {size}
                      </button>
                    ))}
                  </div>
                  <button className={`mt-4 px-2 py-1.5 md:text-sm text-[10px] ${selectedSize ? "bg-black text-white" : "bg-neutral-300"} justify-center w-4/5 flex mx-auto rounded`}
